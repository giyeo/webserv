...
class MyTests(unittest.TestCase):
	...
	def test1GetErrorTest(self):
		response = requests.get("http://www.example.com:8081/nonexe")
		headers = dict(response.headers)
	...
	def test2GetIndex(self):
		...
		self.assertEqual(response.status_code, 200)
		self.assertEqual(headers.get("Server"),"rafinha.com")
		self.assertTrue("test2index" in response.text)
		
	def test2GetError(self):
		response = requests.get("http://testando:8082/nonexist")
		headers = dict(response.headers)
		
		self.assertEqual(response.status_code, 200)
		self.assertEqual(headers.get("Server"),"testando")
